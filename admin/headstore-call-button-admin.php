...
class Headstore_Config_Admin {
	...
	public function enqueue_styles() {
		...
		wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/headstore-call-button-admin.css', array(), $this->version, 'all' );
	...
	public function add_post_type_account() {
		...
		global $wp_version;
		$icon = get_site_url().'/wp-content/plugins/headstore-call-button/images/';
		$icon .=  (version_compare( $wp_version, '3.9' , '<') ) ? 'headstore-icon-blue.png' : 'headstore-icon.png';
	...
	public function display_meta_form_account( $post ) {
		...
		include 'partials/message_check_email.php';
	...
	public function headstore_step_create_account() {
		...
		if (isset($_POST['form-submit'])) {
			...
			if(empty($_POST['email_account'])){
				...
				$this->show_message_error(__('Field Email is required',$this->plugin_name)); 
				$create_register = false;
			}
			
			if(empty($_POST['password_account'])){
				
				$this->show_message_error(__('Field Password is required',$this->plugin_name)); 
				$create_register = false;
			}
			...
			if($create_register) {
				...
				if(isset($result['error'])){
					...
					$this->show_message_error($result['error_description']); 
	...
	public function steps_message() {
	   ...
	   if(isset($_GET['page']) && $_GET['page'] == 'headstore-call-button-admin.php') return;	
	...
	private function url_create_post_with_steps() {
		return get_home_url().'/wp-admin/edit.php?post_type='.account_meta_data::post_type.'&page=headstore-call-button-admin.php';
	}
	...
	private function show_message_error($message) {
		include 'partials/show_message_error.php';
	}
	...
	public function my_admin_notices_save_post() {
	    ...
	    if($hs_message == 0){
	   	    include 'partials/show_message_succeeded.php';
	    } else {
	        include 'partials/show_message_error.php';
	    }
	...
	public function hs_mce_plugin( $plugins ) {
		...
		wp_localize_script(
			...
			array(
				...
				'label_expert' => __( 'Expert:', $this->plugin_name ),
				...
				'id_ajaxurl' => site_url('/').'wp-admin/admin-ajax.php',
				'label_expert_input_title' => __( 'Chose one of your groups. You can create new ones logging in with your account on https://'.hs_backend::domain, $this->plugin_name ),
